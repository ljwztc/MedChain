...
def task4(case_message=None, first_room=None, MDDB_path=None, env="answer", feedback=None, RAG_symptom=None, top_3_case=None, RAG=None):
    if env == "answer":
        doctor_num = 3
        task_obj = "诊断"
        ...
        if RAG == True:
            ...
            with open('./my_prompt/task4_diagnosis_first.txt', 'r', encoding='utf-8') as file:
            ...
            with open('./my_prompt/task4_diagnosis_first_no_rag.txt', 'r', encoding='utf-8') as file:
                case_prompt = file.read().replace('[case now]', patient_con)

        sys_prompt = f"你是一名专业的{first_room}医生，有着丰富的临床诊断经验，你需要根据病人的情况，输出诊断结果。"
        response_diagnosis = doctor_group(doctor_num=doctor_num, task_id=4, user_input=case_prompt,
                                          sys_prompt=sys_prompt,
                                          description=patient_con, task_obj=task_obj, first_room=first_room,
                                          max_tokens=100, doc_RAG=RAG)
        ...
        with open('./my_prompt/rag_case.txt', 'r', encoding='utf-8') as file:
            ...
            for index, row in rows.iterrows():
                ...
                case_prompt = case_prompt.replace(f"[case{case_num}]", case_str)
                case_num += 1
        with open('./my_prompt/task4_diagnosis_feedback.txt', 'r', encoding='utf-8') as file:
            feedback_prompt = file.read().replace("[feedback]", f"{feedback[1]}").replace("[past_case]", f"{case_prompt}")
        # print(feedback_prompt)
        sys_prompt = f"你是一名专业的{first_room}医生，有着丰富的临床诊断经验，你需要根据病人的情况，输出诊断结果。"
        response_diagnosis = gpt_api(max_tokens=1000, system_role=sys_prompt, user_input=feedback_prompt)
